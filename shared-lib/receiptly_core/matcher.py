...
class AmazonStyleMatcher:
    ...
    def _apply_hard_constraints(
        ...
        Constraints:
        1. Pack count mismatch → REJECT (multi-pack vs single pack are different products)
        2. Size mismatch >5% → REJECT
        3. Brand conflict (different brands) → REJECT
        4. Variant mismatch (<50% keyword overlap) → REJECT
        5. Category mismatch (DISABLED - too inconsistent across stores)
        ...
        for candidate in candidates:
            ...
            # Constraint 4: Variant must have significant overlap (if both present)
            # This prevents "Gold Blend" from matching "Classic Decaf"
            if query_attrs.get('variant') and candidate.get('variant'):
                query_variant = query_attrs['variant'].lower()
                candidate_variant = candidate['variant'].lower()
                
                # Extract key variant words (remove common words and sizes)
                def extract_variant_keywords(variant_text):
                    # Remove size patterns and common words
                    import re
                    text = re.sub(r'\d+\.?\d*\s*(g|kg|ml|l|oz)\b', '', variant_text, flags=re.IGNORECASE)
                    words = text.split()
                    # Filter out very common words
                    common_words = {'pack', 'bottle', 'can', 'box', 'jar', 'bag'}
                    return {w for w in words if len(w) > 2 and w not in common_words}
                
                query_keywords = extract_variant_keywords(query_variant)
                candidate_keywords = extract_variant_keywords(candidate_variant)
                ...
                if query_keywords and candidate_keywords:
                    ...
                    #   overlap=1, query_ratio=1/2=50%, candidate_ratio=1/4=25% → REJECT (25% < 50%)
                    # "gold blend" (2) vs "gold roast" (2):
                    #   overlap=1, query_ratio=1/2=50%, candidate_ratio=1/2=50% → ACCEPT (both ≥ 50%)
                    if query_ratio < 0.5 or candidate_ratio < 0.5:
                        continue
            
            # Constraint 5: Category must match (if both present and not unknown)
            # DISABLED: Categories vary too much across stores (e.g., "Beverages" vs "Instant Coffee")
            # Brand + Size + Pack Count + Variant are sufficient hard constraints
